...
cp -r ${REAL_PATH}/* ${TEMPPATH}/real/all
cp -r ${CF_PATH}/* ${TEMPPATH}/cf/all
...
for SPLIT in split1 split2; do
    rsync --files-from=${TEMPPATH}/${SPLIT}.txt --relative ${TEMPPATH}/real/all/ ${TEMPPATH}/real/${SPLIT}
    rsync --files-from=${TEMPPATH}/${SPLIT}.txt --relative ${TEMPPATH}/cf/all/ ${TEMPPATH}/cf/${SPLIT}
done

echo "Computing symmetric FID:"
